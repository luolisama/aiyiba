...
const manager = createTimelineGameManager({ normal: songsJson, hardcore: hardcoreSongsJson });
...
export async function POST(request: Request) {
  ...
  let payload: { action?: string; pool?: string; clientId?: string; roundId?: string; slot?: number };
  try {
  ...
  try {
    switch (payload.action) {
      ...
      case "place": return json({ state: manager.place(payload.roundId, payload.slot) });
